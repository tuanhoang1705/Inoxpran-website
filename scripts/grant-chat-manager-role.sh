...
if [[ -z "$EMAIL" ]]; then
  echo "Usage: bash scripts/grant-chat-manager-role.sh <admin-email>" >&2
  echo "Optional: COMPOSE_FILE=docker-compose.yml bash scripts/grant-chat-manager-role.sh <admin-email>" >&2
  exit 1
fi
