<script>
	...
	const runActivityKeys = new SvelteMap();
	...
	const imageSearchCapability = $derived(resolvedCapability('image_search', 'imageSearch'));
	const aiImageCapability = $derived(resolvedCapability('ai_image', 'aiImage'));
	...
	const pageErrorText = $derived(
		pageError ? requestErrorText(pageError, 'OPENCLAW_CONSOLE_LOAD_FAILED') : ''
	);
	const selectedRunOutput = $derived.by(() => {
		if (!selectedRun) return copy.noOutput;
		const failed = ['failed', 'timed_out'].includes(String(selectedRun.status || '').toLowerCase());
		if (failed || selectedRun.error) {
		...
		return selectedRun.output || copy.noOutput;
	});
	...
	const actionMeta = $derived({
		...
		'daily-draft': {
			label: isEn ? 'Blog scheduling' : 'Lên lịch Blog',
			detail: isEn ? 'Manage blog schedules' : 'Quản lý lịch tạo bài'
		}
	});
	...
	const refreshDashboard = async () => {
		...
		try {
			...
			if (!response.ok) {
				pageError = normalizeOpenClawUiError(
					payload,
			...
			pageError = normalizeOpenClawUiError(null, 'OPENCLAW_CONSOLE_LOAD_FAILED');
		} finally {
			refreshingDashboard = false;
		}
	};

	const syncRunActivity = (run, wasActive = false) => {
		if (!run?.id) return;
		const rawStatus = String(run.status || (wasActive ? 'running' : '')).toLowerCase();
		const isRunning = ['queued', 'running'].includes(rawStatus);
		const existing = openClawActivityStore.find(
			(item) => item.domain === 'bos-console' && item.runId === String(run.id)
		);
		if (!isRunning && !wasActive && existing?.status !== 'running') return;
		const key = runActivityKeys.get(String(run.id)) || existing?.key || `bos-console:${run.id}`;
		runActivityKeys.set(String(run.id), key);
		openClawActivityStore.upsert({
			key,
			domain: 'bos-console',
			entityId: run.action || 'console',
			runId: run.id,
			action: run.action || 'run',
			title: run.label || run.action || 'BOS Console',
			status: openClawActivityStatus(rawStatus),
			rawStatus,
			message: run.error
				? requestErrorText(
						{ errorCode: run.errorCode, requestId: run.requestId },
						'OPENCLAW_REQUEST_FAILED'
					)
	...
	const refreshRuns = async () => {
		if (refreshingRuns) return { active: activeRuns.length > 0 };
		refreshingRuns = true;
		try {
			...
			if (!response.ok) {
				throw normalizeOpenClawUiError(payload, 'OPENCLAW_CONSOLE_LOAD_FAILED', response.headers);
			}
			const nextRuns = Array.isArray(payload?.runs) ? payload.runs : runs;
			for (const run of nextRuns) syncRunActivity(run, previousActive.has(String(run.id)));
			runs = nextRuns;
	...
	const startRun = async (action) => {
		...
		const activityKey = `bos-console:${action}:${idempotencyKey}`;
		openClawActivityStore.upsert({
			key: activityKey,
			domain: 'bos-console',
			entityId: action,
			action,
			title: actionDefinition?.label || action,
			status: 'running',
			rawStatus: 'running',
			message: isEn ? 'Command is running…' : 'Lệnh đang được thực thi…'
		});
		...
		try {
			...
			if (!response.ok) {
				...
				openClawActivityStore.upsert({
					...
					status: 'failed',
					rawStatus: 'failed',
					message: requestErrorText(pageError, 'OPENCLAW_RUN_START_FAILED')
				});
				return;
			}
			if (payload?.id) runActivityKeys.set(String(payload.id), activityKey);
			upsertRun(payload);
			selectedRunId = payload.id;
			syncRunActivity(payload, true);
			consolePoller?.poke('run-started');
		} catch {
			pageError = normalizeOpenClawUiError(null, 'OPENCLAW_RESPONSE_INTERRUPTED');
			openClawActivityStore.upsert({
				key: activityKey,
				status: 'failed',
				rawStatus: 'failed',
				message: requestErrorText(pageError, 'OPENCLAW_RESPONSE_INTERRUPTED')
			});
		} finally {
			busyAction = '';
		}
	...
	onMount(() => {
		...
		consolePoller = createSmartPoller({
			poll: refreshRuns,
			...
			onSyncError: () => {
				if (activeRuns.length) {
					liveSyncMessage = isEn
						? 'Live status sync is delayed. The command is still being tracked.'
						: 'Cập nhật trạng thái đang tạm chậm. Lệnh vẫn đang được theo dõi.';
				}
			}
		});
		consolePoller.start();
		return () => consolePoller?.stop();
	});
</script>
...
<section class="openclaw-console">
	...
	{#if pageErrorText}
		<div class="oc-alert" role="alert">{pageErrorText}</div>
	{/if}
	{#if liveSyncMessage}
		<div class="oc-alert oc-alert--sync" role="status">{liveSyncMessage}</div>
	{/if}
							...
							<button
								...
								onclick={() =>
									action.id === 'daily-draft'
										? goto(resolve(resolveAdminPath('/admin/openclaw/blogs')))
										: startRun(action.id)}
							>
		...
		<aside class="oc-panel oc-panel--safety">
			...
			<div class="oc-gate-group">
				...
				<div class="oc-gate__row">
					...
					<b
						class="oc-badge"
						class:is-good={capabilityTone(imageSearchCapability) === 'good'}
						class:is-warn={capabilityTone(imageSearchCapability) === 'warn'}
						class:is-danger={capabilityTone(imageSearchCapability) === 'danger'}
						class:is-muted={capabilityTone(imageSearchCapability) === 'muted'}
						>{capabilityLabel(imageSearchCapability)}</b
					>
				</div>
				<div class="oc-gate__row">
					<span class="oc-gate__identity"
					...
					<b
						class="oc-badge"
						class:is-good={capabilityTone(aiImageCapability) === 'good'}
						class:is-warn={capabilityTone(aiImageCapability) === 'warn'}
						class:is-danger={capabilityTone(aiImageCapability) === 'danger'}
						class:is-muted={capabilityTone(aiImageCapability) === 'muted'}
						>{capabilityLabel(aiImageCapability)}</b
					>
				</div>
				<div class="oc-gate__row">
					<span class="oc-gate__identity"
		...
		<div class="oc-run-layout">
			...
			<div class="oc-run-output">
				{#if selectedRun}
					...
					<pre class="oc-log">{selectedRunOutput}</pre>
				{:else}
					<div class="oc-empty">
						<svg viewBox="0 0 24 24" fill="none" aria-hidden="true"
							><path
...
<style>
	...
	.oc-alert--sync {
		border-color: rgba(190, 139, 36, 0.32);
		background: rgba(190, 139, 36, 0.08);
		color: #725114;
	}
