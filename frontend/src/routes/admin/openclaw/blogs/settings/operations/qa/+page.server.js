		...
		return {
			...
			error:
				response.ok || response.status === 404
					? null
					: normalizeOpenClawUiError(payload, 'OPENCLAW_QA_LOAD_FAILED', response.headers)
		};
	} catch {
		return {
			ok: false,
			status: 0,
			data: {},
			error: normalizeOpenClawUiError(null, 'OPENCLAW_QA_LOAD_FAILED')
		};
	}
};
...
export const load = async ({ cookies, fetch, url }) => {
	...
	const loadErrors = [listResult.error, ...detailErrors].filter(Boolean);
	return {
