<script>
	...
	const requestErrorText = (error, fallbackCode = 'OPENCLAW_REQUEST_FAILED') =>
		openClawUiErrorText(error, { isEn, fallbackCode });
	const pageErrorText = $derived(
		pageError ? requestErrorText(pageError, 'OPENCLAW_QA_LOAD_FAILED') : ''
	);
	...
	const ACTIVE_QA_BATCH_STATUSES = new Set(['running', 'reviewing', 'remediating']);
	const ACTIVE_QA_CASE_STATUSES = new Set([
		'reserved',
		'queued',
		'running',
		'draft_created',
	...
	const isActiveQaBatch = (status) => ACTIVE_QA_BATCH_STATUSES.has(String(status || ''));
	const isActiveQaCase = (status) => ACTIVE_QA_CASE_STATUSES.has(String(status || ''));
	const isActiveRemediation = (status) => String(status || '') === 'in_progress';
	const qaBatchHasActiveWork = (batch = {}) =>
		isActiveQaBatch(batch.status) ||
		(batch.cases || []).some((item) => isActiveQaCase(item.status)) ||
		(batch.remediationAttempts || []).some((item) => isActiveRemediation(item.status));

	const qaOutcomeMessage = (status) => {
		if (status === 'awaiting_remediation_action') {
			return isEn
				? 'Waiting for a manual remediation action.'
				: 'Đang chờ thao tác khắc phục thủ công.';
		}
		if (status === 'passed') return isEn ? 'QA passed.' : 'QA đã đạt yêu cầu.';
		if (['failed', 'blocked'].includes(status)) return statusLabel(status);
		return statusLabel(status);
	};

	const syncQaEntityActivity = ({ domain, entityId, title, rawStatus, active, wasActive }) => {
		if (!entityId) return;
		const existing = openClawActivityStore.find(
			(item) => item.domain === domain && item.entityId === String(entityId)
		);
		if (!active && !wasActive && existing?.status !== 'running') return;
		openClawActivityStore.upsert({
			key: existing?.key || `${domain}:${entityId}`,
			domain,
			entityId,
			action: 'qa-run',
			title,
			...
			rawStatus,
			message: qaOutcomeMessage(rawStatus)
		});
	};

	const syncQaBatchActivities = (nextBatch, previousBatch = null) => {
		if (!nextBatch?.id) return;
		syncQaEntityActivity({
			domain: 'qa-batch',
			entityId: nextBatch.id,
			title: `QA batch ${nextBatch.id}`,
			rawStatus: nextBatch.status,
			active: isActiveQaBatch(nextBatch.status),
			wasActive: isActiveQaBatch(previousBatch?.status)
		});
		const previousCases = new Map((previousBatch?.cases || []).map((item) => [item.id, item]));
		for (const item of nextBatch.cases || []) {
			const previous = previousCases.get(item.id);
			syncQaEntityActivity({
				domain: 'qa-case',
				entityId: item.id,
				title: item.topicSeed || item.caseKey || `QA case ${item.id}`,
				rawStatus: item.status,
				active: isActiveQaCase(item.status),
				wasActive: isActiveQaCase(previous?.status)
			});
		}
		const previousAttempts = new Map(
			(previousBatch?.remediationAttempts || []).map((item) => [item.id, item])
		);
		for (const item of nextBatch.remediationAttempts || []) {
			const previous = previousAttempts.get(item.id);
			syncQaEntityActivity({
				domain: 'qa-remediation',
				entityId: item.id,
				title: remediationLabel(item),
				rawStatus: item.status,
				active: isActiveRemediation(item.status),
				wasActive: isActiveRemediation(previous?.status)
			});
		}
	};

	const beginQaActivity = ({ domain = 'qa-batch', entityId, action, title }) => {
		const key = `${domain}:${entityId || 'collection'}:${action}:${Date.now()}`;
		openClawActivityStore.upsert({
			key,
			domain,
			entityId,
	...
	const finishQaActivity = (key, status, message) => {
	...
	const createBatch = async () => {
		...
		busyAction = 'create';
		const activityKey = beginQaActivity({
			domain: 'qa-action',
			entityId: createEnvironment,
			action: 'create',
			title: isEn ? 'Create QA batch' : 'Tạo QA batch'
		});
		try {
			...
			showSuccess('created');
			finishQaActivity(activityKey, 'succeeded', notice?.text || 'QA batch created');
		} catch (error) {
			showError(error);
			finishQaActivity(activityKey, 'failed', notice?.text || requestErrorText(error));
		} finally {
			busyAction = '';
		}
	};
	...
	const performBatchAction = async (action) => {
		...
		const activityKey = beginQaActivity({
			...
			action,
			title: `QA batch ${selectedBatch.id} · ${action}`
		});
		try {
			...
			if (qaBatchHasActiveWork(selectedBatch || {})) {
				openClawActivityStore.upsert({
					key: activityKey,
					status: 'running',
					rawStatus: selectedBatch.status,
					message: notice?.text || qaOutcomeMessage(selectedBatch.status)
				});
				qaPoller?.poke(`qa-${action}`);
			} else {
				finishQaActivity(
					activityKey,
					openClawActivityStatus(selectedBatch?.status),
					notice?.text || qaOutcomeMessage(selectedBatch?.status)
				);
			}
		} catch (error) {
			showError(error);
	...
	const resumeArticleRemediation = async (attempt) => {
		...
		const activityKey = beginQaActivity({
			...
			entityId: attemptId,
			action: 'resume',
			title: remediationLabel(attempt)
		});
		try {
			await api(
			...
			const refreshedAttempt = selectedBatch?.remediationAttempts?.find(
				(item) => item.id === attemptId
			);
			if (isActiveRemediation(refreshedAttempt?.status)) {
				openClawActivityStore.upsert({
					key: activityKey,
					status: 'running',
					rawStatus: refreshedAttempt.status,
					message: notice?.text || qaOutcomeMessage(refreshedAttempt.status)
				});
				qaPoller?.poke('qa-remediation-resume');
			} else {
				finishQaActivity(
					activityKey,
					openClawActivityStatus(refreshedAttempt?.status),
					notice?.text || qaOutcomeMessage(refreshedAttempt?.status)
				);
			}
		} catch (error) {
			showError(error);
	...
	const pollQa = async () => {
		const previousBatches = new Map(batches.map((batch) => [batch.id, batch]));
		const previousSelected = selectedBatch;
		const payload = await api('', { query: '?limit=20&page=1' });
		const nextBatches = qaBatchList(payload);
		batches = nextBatches;
		accessPayload = payload;
		const refreshedEnvironment = String(payload?.environment || '').trim();
		if (['local', 'staging'].includes(refreshedEnvironment)) {
		...
		for (const batch of nextBatches) {
			syncQaBatchActivities(batch, previousBatches.get(batch.id));
		}

		const selectedId = qaEntityId(selectedBatch?.id || nextBatches[0]?.id);
		let detailedBatch = null;
		if (selectedId) {
			const [detail, reportPayload] = await Promise.all([
				api(encodeURIComponent(selectedId)),
				api(`${encodeURIComponent(selectedId)}/reports`)
			]);
			detailedBatch = normalizeQaBatchDetail(detail);
			applyBatchPayload(detail);
			const reportItems = Array.isArray(reportPayload)
				? reportPayload
				: reportPayload?.reports || reportPayload?.items || [];
			reports = (Array.isArray(reportItems) ? reportItems : []).map(normalizeQaReport);
			syncQaBatchActivities(
				detailedBatch,
				previousSelected?.id === selectedId ? previousSelected : null
			);
		}
		...
		return {
			active:
				nextBatches.some((batch) => isActiveQaBatch(batch.status)) ||
				qaBatchHasActiveWork(detailedBatch || {})
		};
	};

	onMount(() => {
		openClawActivityStore.hydrate();
		qaPoller = createSmartPoller({
			poll: pollQa,
			...
			onSyncError: () => {
				if (
					batches.some((batch) => isActiveQaBatch(batch.status)) ||
					qaBatchHasActiveWork(selectedBatch || {})
				) {
					liveSyncMessage = isEn
						? 'Live QA status sync is delayed. The batch is still being tracked.'
						: 'Cập nhật trạng thái QA đang tạm chậm. Batch vẫn được theo dõi.';
				}
			}
		});
		qaPoller.start();
		return () => qaPoller?.stop();
	});
</script>
...
<div class="qa-page">
	<header class="qa-hero">
		<div>
			<nav aria-label={$t('admin.contentOperations.qa.breadcrumbLabel')}>
				...
				<a href={resolve(resolveAdminPath('/admin/openclaw/blogs/settings/operations'))}
	...
	{#if pageErrorText}
		<p class="qa-alert qa-alert--danger" role="alert">{pageErrorText}</p>
	{/if}
	{#if liveSyncMessage}
		<p class="qa-alert qa-alert--sync" role="status">{liveSyncMessage}</p>
	{/if}
	{#if notice}
		<div
