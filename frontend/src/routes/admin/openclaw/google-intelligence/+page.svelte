<script>
	...
	const t = $derived({
		...
		noChanges: isEn ? 'No material changes recorded for this snapshot.' : 'Không có thay đổi đáng kể trong snapshot này.',
		...
		sourceHint: isEn ? 'Official sources remain authoritative; third-party sources are interpretation only.' : 'Nguồn chính thức giữ vai trò quyết định; nguồn bên thứ ba chỉ là diễn giải.',
		...
		overrideWarning: isEn ? 'An override opens the gate but preserves the failed status in the audit log.' : 'Override sẽ mở cổng nhưng vẫn giữ trạng thái lỗi trong audit log.',
		...
		styleHint: isEn ? 'Rotate structure, not just vocabulary. A complete reusable article template cannot be defined here.' : 'Luân phiên cấu trúc, không chỉ từ ngữ. Không thể định nghĩa mẫu bài hoàn chỉnh để lặp lại tại đây.',
	...
	const resolveAdminPath = (path) => {
		if (typeof window === 'undefined' || window.location.hostname !== 'admin.inoxpran.com') return path;
	...
	const formatDate = (value) => {
		...
		return new Intl.DateTimeFormat(isEn ? 'en-US' : 'vi-VN', { dateStyle: 'medium', timeStyle: 'short' }).format(date);
	};
	...
	const refreshAll = async () => {
		busy = 'refresh'; error = ''; notice = '';
		try {
			const [nextStatus, nextSources, nextSnapshots, nextSchedule, nextExecutions, nextBlogs, nextStyles] = await Promise.all([
				api('google-intelligence/status'), api('google-intelligence/sources'), api('google-intelligence/snapshots?limit=20'),
				api('google-intelligence/schedule'), api('google-intelligence/executions?limit=30'),
				api('google-intelligence/related-blogs?limit=30'), api('editorial-styles/')
			]);
			status = nextStatus; sources = nextSources.sources || []; snapshots = nextSnapshots.snapshots || [];
			schedule = nextSchedule; executions = nextExecutions.executions || []; relatedBlogs = nextBlogs.blogs || [];
			styles = nextStyles.styles || []; recentProfiles = nextStyles.recentProfiles || [];
			notice = t.success;
	...
	const runNow = async () => {
		busy = 'run'; error = ''; notice = '';
		try { await api('google-intelligence/run-now', { method: 'POST', body: '{}' }); notice = t.success; await refreshAll(); }
		catch (cause) { error = cause.message; busy = ''; }
	};

	const updateSource = async (source, patch) => {
		busy = `source-${source.id}`; error = ''; notice = '';
		try {
			const updated = await api(`google-intelligence/sources/${source.id}`, { method: 'PATCH', body: JSON.stringify(patch) });
			sources = sources.map((item) => item.id === source.id ? updated : item); notice = t.success;
		} catch (cause) { error = cause.message; }
		finally { busy = ''; }
	};
	...
	const runSource = async (source) => {
		busy = `source-${source.id}`; error = ''; notice = '';
		try { await api(`google-intelligence/sources/${source.id}/run-now`, { method: 'POST', body: '{}' }); notice = t.success; await refreshAll(); }
		catch (cause) { error = cause.message; busy = ''; }
	};
	...
	const overrideLatest = async () => {
		if (!latest?.id) return;
		busy = 'override'; error = ''; notice = '';
		try {
			const updated = await api(`google-intelligence/snapshots/${latest.id}/override`, { method: 'POST', body: JSON.stringify({ reason: overrideReason }) });
			status = { ...status, snapshot: updated, gateOpen: true }; overrideReason = ''; notice = t.success;
		} catch (cause) { error = cause.message; }
		finally { busy = ''; }
	};

	const updateStyle = async (style, patch) => {
		busy = `style-${style.id}`; error = ''; notice = '';
		try {
			const updated = await api(`editorial-styles/${style.id}`, { method: 'PATCH', body: JSON.stringify(patch) });
			styles = styles.map((item) => item.id === style.id ? updated : patch.locked ? { ...item, locked: false } : item); notice = t.success;
		} catch (cause) { error = cause.message; }
		finally { busy = ''; }
	};

	const generateTodayStyle = async () => {
		busy = 'style-generate'; error = ''; notice = '';
		try { await api('editorial-styles/generate-today', { method: 'POST', body: '{}' }); notice = t.success; await refreshAll(); }
		catch (cause) { error = cause.message; busy = ''; }
	};
</script>
...
<section class="intel-shell">
	<a class="back" href="/admin/openclaw">← {t.back}</a>
	...
	<header class="hero">
		...
		<div class="hero-actions">
			<button class="button secondary" onclick={refreshAll} disabled={Boolean(busy)}>{t.refresh}</button>
			<button class="button primary" onclick={runNow} disabled={Boolean(busy)}>{busy === 'run' ? t.running : t.runNow}</button>
		</div>
	</header>
	...
	<section class="block overview">
		<div class="section-title"><span>01</span><div><h2>{t.today}</h2><p>{latest?.snapshotDate || '—'} · {latest?.timezone || schedule?.timezone || 'Asia/Ho_Chi_Minh'}</p></div></div>
		<div class="metric-grid">
			<article class="metric"><span>{t.snapshot}</span><strong class:danger-text={latest?.status === 'failed'}>{statusLabel(latest?.status)}</strong><small>{t.checked}: {formatDate(latest?.checkedAt)}</small></article>
			<article class="metric gate" class:closed={!status?.gateOpen}><span>{t.gate}</span><strong>{status?.gateOpen ? t.open : t.blocked}</strong><small>{status?.strictGate ? 'STRICT' : 'ADVISORY'}</small></article>
			<article class="metric"><span>{t.sourceHealth}</span><strong>{latest?.successfulSources || 0}/{latest?.sourcesChecked || 0}</strong><small>{latest?.failedSources || 0} {isEn ? 'failed' : 'lỗi'}</small></article>
			<article class="metric"><span>{t.changes}</span><strong>{changes.length}</strong><small>{latest?.noMaterialChanges ? 'NO MATERIAL CHANGE' : 'REVIEW REQUIRED'}</small></article>
			<article class="metric"><span>{t.critical}</span><strong>{(latest?.risks || []).filter((item) => item.severity === 'critical').length}</strong><small>{status?.latestRun?.status || '—'}</small></article>
		</div>
	</section>

	<div class="split">
		<section class="block">
			<div class="section-title compact"><span>02</span><div><h2>{t.latestChanges}</h2></div></div>
			{#if changes.length}
		...
		<section class="block guidance">
			<div class="section-title compact"><span>03</span><div><h2>{t.currentGuidance}</h2></div></div>
			{#each [[t.technicalSeo, guidance.technicalSeo], [t.content, guidance.content], [t.aiSearch, guidance.aiSearch], [t.structuredData, guidance.structuredData], [t.spamRisk, guidance.spamRisk]] as item}
				<div class="guidance-row"><h3>{item[0]}</h3><p>{item[1] || '—'}</p></div>
			{/each}
		</section>
	</div>

	<section class="block">
		<div class="section-title"><span>04</span><div><h2>{t.sources}</h2><p>{t.sourceHint}</p></div></div>
	...
	<section class="block">
		<div class="section-title"><span>05</span><div><h2>{t.schedule}</h2><p>{schedule?.nextRunAt ? `${isEn ? 'Next' : 'Tiếp theo'}: ${formatDate(schedule.nextRunAt)}` : '—'}</p></div></div>
		<div class="form-grid">
	...
	<div class="split histories">
		<section class="block"><div class="section-title compact"><span>06</span><div><h2>{t.executions}</h2></div></div><div class="timeline">
			{#each executions.slice(0, 12) as execution}<article><span class="timeline-dot" class:failed={execution.status === 'failed'}></span><div><strong>{statusLabel(execution.status)}</strong><p>{execution.triggeredBy} · {execution.changesDetected || 0} changes</p><small>{formatDate(execution.startedAt)} {execution.error ? `· ${execution.error}` : ''}</small></div></article>{/each}
		</div></section>
		<section class="block"><div class="section-title compact"><span>07</span><div><h2>{t.snapshots}</h2></div></div><div class="snapshot-list">
			{#each snapshots.slice(0, 12) as item}<article><div><strong>{item.snapshotDate}</strong><p>{statusLabel(item.status)}</p></div><div><b>{item.successfulSources}/{item.sourcesChecked}</b><small>{formatDate(item.checkedAt)}</small></div></article>{/each}
		</div></section>
	</div>

	<section class="block override">
		<div class="section-title"><span>08</span><div><h2>{t.manualOverride}</h2><p>{t.overrideWarning}</p></div></div>
		<div class="override-form"><textarea rows="3" bind:value={overrideReason} placeholder={t.reason}></textarea><button class="button danger-button" onclick={overrideLatest} disabled={busy === 'override' || overrideReason.trim().length < 10 || !latest?.id}>{t.override}</button></div>
	</section>

	<section class="block">
		<div class="section-title"><span>09</span><div><h2>{t.styles}</h2><p>{t.styleHint}</p></div><button class="button secondary" onclick={generateTodayStyle} disabled={Boolean(busy)}>{t.generateStyle}</button></div>
		<div class="style-grid">{#each styles as style (style.id)}<article class:disabled={!style.enabled} class:locked={style.locked}>
			<div class="style-head"><h3>{style.styleFamily}</h3><button class="toggle" class:on={style.enabled} aria-label={`${t.enabled} ${style.styleFamily}`} onclick={() => updateStyle(style, { enabled: !style.enabled })}><span></span></button></div>
			<p>{style.description}</p><div class="style-controls"><label>{t.cooldown}<input type="number" min="1" max="30" value={style.cooldownDays} onchange={(event) => updateStyle(style, { cooldownDays: Number(event.currentTarget.value) })} /></label><button class="lock" class:active={style.locked} onclick={() => updateStyle(style, { locked: !style.locked })}>⌁ {t.locked}</button></div>
			<small>{isEn ? 'Last used' : 'Dùng gần nhất'}: {formatDate(style.lastUsedAt)} · {style.useCount || 0}×</small>
		</article>{/each}</div>
		<h3 class="subhead">{t.recentProfiles}</h3><div class="profile-strip">{#each recentProfiles.slice(0, 14) as profile}<span><b>{profile.date}</b>{profile.styleFamily}<small>{profile.openingMode} · {profile.headingMode}</small></span>{/each}</div>
	</section>

	<div class="split">
		<section class="block"><div class="section-title compact"><span>10</span><div><h2>{t.relatedBlogs}</h2></div></div><div class="blog-list">{#each relatedBlogs as item}<article><div><strong>{item.title}</strong><p>{item.googleIntelSnapshotDate} · {item.googleIntelStatus}</p><small>style {item.editorialStyleProfileId.slice(-6)} · strategy {item.strategyPlanId.slice(-6)}</small></div><a href={`/admin/blogs/${item.id}`} target="_blank" rel="noreferrer">{t.openEditor} ↗</a></article>{/each}</div></section>
