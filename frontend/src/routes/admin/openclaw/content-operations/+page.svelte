<script>
	...
	const api = async (path, options = {}) => {
		...
		if (!response.ok)
			throw new Error(payload?.error || $t('admin.contentOperations.feedback.requestFailed'));
		return payload || {};
	};
		...
		notice = {
			...
			text:
				error instanceof Error
					? error.message
					: $t('admin.contentOperations.feedback.requestFailed')
		};
	};
	...
	const handleOpportunity = async (item, operation, details = {}) => {
		...
		if (!isSafeEntityId(id))
			return showError(new Error($t('admin.contentOperations.feedback.invalidId')));
		busyId = id;
		try {
	...
	const handleWorkOrderRun = async (item) => {
		...
		try {
			...
			workOrders = {
				...workOrders,
				workOrders: replaceById(workOrderItems, id, result?.workOrder || { status: 'running' })
			};
...
<div class="operations-page">
	<header class="operations-hero">
		<div class="hero-copy">
			<div class="breadcrumb">
				<a href={resolve(resolveAdminPath('/admin/openclaw'))}>OpenClaw</a><span>/</span><strong
					>{$t('admin.contentOperations.shortTitle')}</strong
				>
			</div>
	...
	{#if failedLoads.length}
		<div class="load-warning" role="status">
			{$t('admin.contentOperations.feedback.partialLoad', { count: failedLoads.length })}
		</div>
	{/if}
	{#if notice}
		<div class="page-notice" class:page-notice--danger={notice.tone === 'danger'} role="status">
