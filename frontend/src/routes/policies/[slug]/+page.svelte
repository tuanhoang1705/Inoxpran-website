<script>
	...
	import {
		getPolicyContactLinks,
		getPolicyPath,
		listPolicyDetails
	} from '$lib/content/policies.js';
	...
	const siteUrl = $derived(
		String(env.PUBLIC_SITE_URL || 'https://inoxpran.com')
			.trim()
			.replace(/\/+$/, '') || 'https://inoxpran.com'
	);
	const canonicalUrl = $derived(
		String(data?.canonicalUrl || '').trim() || `${siteUrl}${$page.url.pathname}`
	);
		...
		JSON.stringify({
			...
			itemListElement: [
				...
				},
				{
					'@type': 'ListItem',
					position: 2,
					name: currentLocale === 'en' ? 'Policies' : 'Chính sách',
					item: `${siteUrl}${allPoliciesHref}`
				},
...
<JsonLd value={policyPageJsonLd} />

{#if policy}
	<section class="policy-detail-shell padding-large">
		<div class="container policy-detail-layout">
			<nav
				class="policy-detail-breadcrumb"
				aria-label={currentLocale === 'en' ? 'Breadcrumb' : 'Điều hướng trang'}
			>
				<a href={resolve(currentLocale === 'en' ? '/en' : '/')}
					>{currentLocale === 'en' ? 'Home' : 'Trang chủ'}</a
				>
				<span>/</span>
				<a href={resolve(allPoliciesHref)}>{currentLocale === 'en' ? 'Policies' : 'Chính sách'}</a>
				<span>/</span>
			...
			<div class="policy-detail-grid">
				<div class="policy-detail-main">
					{#each policy.sections as section, __eachIndex1 (section?._id ?? section?.id ?? __eachIndex1)}
						<section class="policy-detail-card">
							<h2>{section.title}</h2>
							<ul>
								{#each section.items as item, __eachIndex3 (item?._id ?? item?.id ?? __eachIndex3)}
									<li>{item}</li>
								{/each}
							</ul>
						</section>
				...
				<aside class="policy-detail-aside">
					<section class="policy-detail-card policy-detail-card--contact">
						...
						<a href={phoneHref}
							><span>{currentLocale === 'en' ? 'Hotline' : 'Hotline'}</span><strong
								>{SITE_CONTACT.phone}</strong
							></a
						>
					...
					<section class="policy-detail-card">
						...
						<nav class="policy-detail-links">
							{#each relatedPolicies as item, __eachIndex2 (item?._id ?? item?.id ?? __eachIndex2)}
								<a href={resolve(item.href)}>{item.shortTitle}</a>
							{/each}
						</nav>
					</section>
				</aside>
...
<style>
	...
	.policy-detail-card--contact a:first-of-type {
		border-top: none;
		padding-top: 0;
	}
	.policy-detail-card--contact span {
		font-size: 0.8rem;
		letter-spacing: 0.08em;
		text-transform: uppercase;
		color: rgba(15, 23, 42, 0.58);
	}
	.policy-detail-card--contact strong {
		font-size: 1rem;
	}
	.policy-detail-links {
		display: grid;
	...
	.policy-detail-links a:hover {
		border-color: rgba(10, 138, 161, 0.28);
		color: #0a8aa1;
	}
