<script>
	...
	const contactReturnUrl = $derived(page.url.pathname + page.url.search);
	const contactStatus = $derived(page.url.searchParams.get('contact'));
	const privacyPolicyHref = $derived(localizeInternalHref('/policies/privacy-policy', $locale));
	...
	const normalizeToastMessage = (value) =>
		String(value || '')
			.replace(/\s+/g, ' ')
			.trim();
	const truncateToastMessage = (value, maxLength = 110) => {
		const text = normalizeToastMessage(value);
		if (text.length <= maxLength) return text;
		return `${text.slice(0, Math.max(0, maxLength - 3)).trimEnd()}...`;
	};
	const showFooterToast = (message, type = 'success') => {
		const safeMessage = truncateToastMessage(message, type === 'success' ? 96 : 120);
		cartToast.show(safeMessage, type, type === 'success' ? 2200 : 2800);
	};
	...
	$effect(() => {
		if (contactStatus === 'success') {
			showFooterToast($t('footer.contactSubmitSuccess'), 'success');
			clearContactQuery();
		} else if (contactStatus === 'error') {
			showFooterToast($t('footer.contactSubmitError'), 'danger');
			clearContactQuery();
		}
	});

	const handleFooterContactSubmit = async (event) => {
		...
		try {
			...
			if (!response.ok || payload?.success === false) {
				showFooterToast(payload?.message || $t('footer.contactSubmitError'), 'danger');
				return;
			}
		...
		<div class="container newsletter-content">
			<div class="contact-form-wrapper">
				<div class="contact-header">
					<h2>{$t('footer.consultTitle')}</h2>
					<p class="sub-text">{$t('footer.consultDesc')}</p>
				</div>
				...
				<form
					...
					onsubmit={handleFooterContactSubmit}
					aria-busy={isSubmittingContact}
				>
					<input type="hidden" name="returnTo" value={contactReturnUrl} />
					<div class="contact-form-left">
						<div class="field">
							<label for="name">{$t('footer.fullName')}</label>
							<input
								id="name"
								name="name"
								type="text"
								placeholder={$t('footer.fullNamePlaceholder')}
								required
							/>
						</div>
						<div class="field">
							<label for="contact">{$t('footer.contactField')}</label>
							<input
								id="contact"
								name="contact"
								type="text"
						...
						<div class="field">
							<label for="message">{$t('footer.request')}</label>
							<textarea
								id="message"
								name="message"
								rows="3"
								placeholder={$t('footer.requestPlaceholder')}
								required
							></textarea>
						</div>
			...
			<div class="newsletter-image">
				<img
					src="/images/optimized/gift-img-720.png"
					srcset="/images/optimized/gift-img-360.png 360w, /images/optimized/gift-img-720.png 720w"
					sizes="(max-width: 768px) 70vw, 360px"
					width="720"
					height="720"
					loading="lazy"
					decoding="async"
					alt={$t('footer.supportImageAlt')}
				/>
			</div>
		</div>
	</div>
	...
	<div class="main-footer-links">
		<div class="container links-grid">
			...
			<div class="footer-column">
				...
				<ul>
					<li><span>{$t('footer.guidePurchase')}</span></li>
					<li><span>{$t('footer.guidePayment')}</span></li>
					<li><span>{$t('footer.guideDelivery')}</span></li>
				</ul>
			</div>
		</div>
	</div>
		...
		<div class="container bottom-content">
			<div class="hotline-box">
				...
				<div class="phone-info">
					<span class="phone-number">{SITE_CONTACT.phone}</span>
					<span class="work-time">{$t('footer.workTime')}</span>
				</div>
			</div>
