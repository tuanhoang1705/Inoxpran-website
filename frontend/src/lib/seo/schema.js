...
const resolveAggregateRating = (product) => {
	const explicitAverage = normalizeRating(
		product?.product_ratingsAverage ?? product?.rating?.average
	);
	const explicitCount = Math.max(
		0,
		Math.floor(
			Number(product?.product_ratingsCount ?? product?.rating?.count ?? product?.reviewCount) || 0
		)
	);
	...
	const average = approvedRatings.reduce((sum, value) => sum + value, 0) / approvedRatings.length;
