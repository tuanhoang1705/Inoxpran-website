...
const { runImagePipeline } = require('../src/services/openclaw/imagePipeline.service');
const { buildImagePrompt } = require('../src/services/openclaw/imagePromptBuilder.service');
const { optimizeImage } = require('../src/services/openclaw/imageOptimize.service');
const { searchImages } = require('../src/services/openclaw/imageSearch.service');
const {
    buildVisualPlan,
    detectArticleType
...
describe('image SEO utilities', () => {
    ...
    it('inserts a semantic figure after the selected heading only once', () => {
        ...
        expect(result.indexOf('Lau bang khan mem')).toBeLessThan(result.indexOf('<figure>'));
        expect(result.match(/<figure>/g)).toHaveLength(1);
        expect(insertInlineImages(result, [{
            url: '/images/test.webp',
            headingIndex: 1
        }]).match(/<figure>/g)).toHaveLength(1);
    });
...
describe('provider and pipeline fallback', () => {
    it('builds realistic prompts with explicit negative guardrails', () => {
        ...
        expect(prompt.positivePrompt).toContain('Realistic Vietnamese home kitchen');
    ...
    it('skips missing providers without throwing', async () => {
        await expect(searchImages({ query: 'stainless cookware' })).resolves.toMatchObject({
            status: 'skipped',
            reason: 'image_search_disabled'
        });
        await expect(generateImage({ prompt: { positivePrompt: 'test' } })).resolves.toMatchObject({
            status: 'skipped',
