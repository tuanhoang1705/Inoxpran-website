...
const isSafeConfiguredBoolean = (key, value) =>
    typeof value === 'boolean' && /configured$/i.test(String(key || ''));
...
const safeErrorCode = (error) => String(error?.code || error?.name || 'CAPABILITY_PROBE_FAILED')
...
const normalizeRuntime = (runtime = {}) => sanitizeSafeValue({
    ...emptyRuntime(),
    ...runtime,
...
const availabilityForStatus = (status) => {
    ...
    if (status === 'degraded' || status === 'manual_review') return 'degraded';
    if (status === 'failed' || status === 'missing_config' || status === 'unavailable') return 'unavailable';
    return 'unknown';
};

const schedulerRuntimeIssue = ({ scheduler = {}, workloadKey, now = new Date(), nextRunAt = null } = {}) => {
    ...
    const pollIntervalMs = Number.isFinite(Number(scheduler.pollIntervalMs))
        ? Math.max(1000, Number(scheduler.pollIntervalMs))
        : DEFAULT_CACHE_MS;
...
const canonicalCapability = (definition, overrides = {}) => {
    ...
    const reasonCode = String(overrides.reasonCode || '').replace(/[^A-Za-z0-9_-]/g, '_').slice(0, 120).toLowerCase();
    const warningList = Array.isArray(overrides.warnings)
        ? overrides.warnings
        : overrides.warnings
            ? [overrides.warnings]
            : [];
    return {
        ...
        lastCheckedAt,
        latencyMs: checked && Number.isFinite(Number(overrides.latencyMs))
            ? Math.max(0, Math.round(Number(overrides.latencyMs)))
            : null,
...
const capabilityStatus = ({
    featureKey = 'unknown',
    labelKey = 'openclaw.capabilities.unknown',
    enabled,
    configured = true,
    ...
    const resolvedStatus = status || (!definition.enabled
        ? (expectedDisabled ? 'expected_disabled' : 'disabled')
    ...
    return canonicalCapability(definition, {
        ...
        reasonCode: reasonCode || (resolvedStatus === 'missing_config'
            ? (definition.configurationValid ? 'missing_configuration' : 'invalid_boolean_configuration')
            : resolvedStatus === 'expected_disabled'
                ? 'safe_rollout_disabled'
                : resolvedStatus === 'disabled'
                    ? 'disabled_by_configuration'
                    : resolvedStatus === 'pending_check'
                        ? 'check_pending'
                        : 'runtime_verified'),
        lastCheckedAt,
        latencyMs,
...
const buildCapabilityDefinitions = (env = process.env, telegramStatus = TelegramApprovalService.status()) => {
    ...
    const telegramMode = String(telegramStatus?.mode || env.TELEGRAM_MODE || 'webhook').trim().toLowerCase() === 'polling'
        ? 'polling'
        : 'webhook';
    ...
    const telegramConfigured = Boolean(
        ...
        (telegramMode !== 'webhook' || telegramStatus?.webhookSecretConfigured)
    );

    const analyticsFlagKey = env.CONTENT_ANALYTICS_ENABLED !== undefined
        ? 'CONTENT_ANALYTICS_ENABLED'
        : 'CONTENT_AGGREGATE_ANALYTICS_ENABLED';
    const config = {
        seoAgent: booleanCapabilityConfig(env, 'SEO_AGENT_ENABLED', {
            configured: [env.API_KEY, env.SEO_AGENT_API_KEY, env.SEO_AGENT_HMAC_SECRET, env.OPENAI_API_KEY].every(hasConfiguredValue)
        }),
        ...
        searchConsole: booleanCapabilityConfig(env, 'SEARCH_CONSOLE_ENABLED', { configured: searchConsoleConfigured }),
        aggregateAnalytics: booleanCapabilityConfig(env, analyticsFlagKey),
        trends: booleanCapabilityConfig(env, 'CONTENT_TRENDS_ENABLED', { configured: trendsProviderConfigured }),
        inventory: booleanCapabilityConfig(env, 'CONTENT_INVENTORY_ENABLED', { fallback: true }),
        contentSignals: booleanCapabilityConfig(env, 'CONTENT_SIGNALS_ENABLED', { fallback: true }),
        ...
        imageSearch: { enabled: providerConfigured(env.IMAGE_SEARCH_PROVIDER), configured: imageSearchConfigured },
        aiImage: { enabled: providerConfigured(env.AI_IMAGE_PROVIDER), configured: aiImageConfigured },
        telegram: booleanCapabilityConfig(env, 'TELEGRAM_BOT_ENABLED', { configured: telegramConfigured }),
        openclawGateway: { enabled: true, configured: hasConfiguredValue(env.OPENCLAW_GATEWAY_TOKEN) }
    };

    return Object.fromEntries(
        Object.entries(CAPABILITY_META).map(([dashboardKey, meta]) => [dashboardKey, {
            dashboardKey,
            ...meta,
            ...config[dashboardKey],
            configDetails: dashboardKey === 'telegram'
                ? {
                    mode: telegramMode,
...
const buildCapabilityMatrix = (env = process.env, telegramStatus = TelegramApprovalService.status()) => Object.fromEntries(
    Object.entries(buildCapabilityDefinitions(env, telegramStatus)).map(([dashboardKey, definition]) => [
        dashboardKey,
        capabilityStatus(definition)
    ])
);

const getGatewayHttpUrl = (env = process.env) => {
    const dockerManaged = String(env.OPENCLAW_DEPLOYMENT_MODE || '').trim().toLowerCase() === 'docker';
...
const probeOpenClawGateway = async ({ fetchImpl = global.fetch, timeoutMs = 5000, env = process.env } = {}) => {
    ...
    if (typeof fetchImpl !== 'function') {
        return { reachable: false, live: false, ready: false, checkedAt, error: 'fetch_not_available' };
    }
    const baseUrl = getGatewayHttpUrl(env);
    if (!baseUrl) return { reachable: false, live: false, ready: false, checkedAt, error: 'gateway_url_invalid' };
...
class CapabilityHealthService {
    constructor({
        ...
        telegramStatusProvider = () => TelegramApprovalService.status(),
        schedulerRuntimeProvider = () => getBlogAutomationSchedulerRuntime(),
        gatewayProbe = (options) => probeOpenClawGateway(options),
        probeRegistry = {},
    ...
    get cacheMs() {
        ...
        return boundedDuration(
            this.cacheMsOverride ?? Number(env.OPENCLAW_CAPABILITY_HEALTH_CACHE_SECONDS) * 1000,
            DEFAULT_CACHE_MS,
    ...
    get timeoutMs() {
        const env = this.envProvider();
        return boundedDuration(
            this.timeoutMsOverride ?? env.OPENCLAW_CAPABILITY_HEALTH_TIMEOUT_MS,
            DEFAULT_TIMEOUT_MS,
            { min: 1, max: 60 * 1000 }
        );
    }
    ...
    resolveDefinition(featureKey) {
        ...
        const definition = Object.values(this.definitions()).find((candidate) => (
            candidate.featureKey === requested || candidate.dashboardKey === requested
        ));
    ...
    async getStatus({ force = false, featureKeys = null } = {}) {
        ...
        const requested = Array.isArray(featureKeys) && featureKeys.length
            ? featureKeys.map((featureKey) => this.resolveDefinition(featureKey))
            : Object.values(definitions);
        const results = await Promise.all(requested.map((definition) => this._checkDefinition(definition, { force })));
        const capabilities = Object.fromEntries(results.map((capability) => {
    ...
    async _checkDefinition(definition, { force = false } = {}) {
        ...
        if (definition.configurationValid === false) {
            return this._cacheResult(definition, signature, canonicalCapability(definition, {
                checked: true,
        ...
        if (!healthConfiguration.valid) {
            return canonicalCapability(definition, {
                ...
                status: 'unavailable',
                reasonCode: 'invalid_boolean_configuration',
                expectedState: definition.enabled ? 'enabled' : 'disabled',
                lastCheckedAt: this.now(),
                latencyMs: 0,
                runtime: { serviceRegistered: null },
                warnings: ['invalid_boolean_configuration'],
        ...
        if (!this.healthChecksEnabled() && definition.enabled && definition.configured) {
            return canonicalCapability(definition, {
                checked: true,
                status: 'unavailable',
                ...
                latencyMs: 0,
                runtime: { serviceRegistered: null },
                warnings: ['capability_health_checks_disabled'],
    ...
    async _executeProbe(definition) {
        ...
        const settledProbe = Promise.resolve()
            ...
            .then((result) => ({ type: 'result', result }), (error) => ({ type: 'error', error }));
        ...
        try {
            ...
            if (outcome.type === 'timeout') {
                return canonicalCapability(definition, {
                    ...
                    latencyMs,
                    runtime: { serviceRegistered: true },
                    warnings: ['capability_probe_timeout']
                });
            }
            if (outcome.type === 'error') {
                return canonicalCapability(definition, {
                    ...
                    latencyMs,
                    runtime: { serviceRegistered: true },
                    safeDetails: { errorCode: safeErrorCode(outcome.error) }
                });
            }
            ...
            return canonicalCapability(definition, {
                ...
                reasonCode: validProbeStatus
                    ? (result.reasonCode || (status === 'healthy' ? 'runtime_verified' : status))
                    : 'invalid_probe_result',
                messageKey: result.messageKey,
                lastCheckedAt: checkedAt,
                latencyMs,
                runtime: result.runtime,
                warnings: validProbeStatus ? result.warnings : ['invalid_probe_result'],
    ...
    _schedulerRuntime() {
        return sanitizeSafeValue(this.schedulerRuntimeProvider() || {});
    }

    async _probeExecutionHistory() {
        const successfulStatuses = ['draft_created', 'maintenance_created', 'published', 'completed', 'skipped'];
    ...
    async _probeContentOperations() {
        ...
        return {
            status,
            reasonCode: snapshotReady
                ? 'runtime_verified'
                : hasWarnings
                    ? 'persisted_snapshot_warning'
                    : snapshot?.status === 'complete' && snapshotFreshness.stale
                        ? 'snapshot_stale'
                    : snapshot
    ...
    async _probeBlogCron() {
        const now = this.now();
        const [schedule, execution, dueTaskCount, enabledScheduleCount] = await Promise.all([
        ...
        const scheduler = this._schedulerRuntime();
        ...
        return {
            ...
            runtime: {
                ...
                lastSuccessfulRunAt: schedule?.lastRunAt || scheduler.lastSuccessfulRunAt || execution?.completedAt || null,
                nextRunAt: schedule?.nextRunAt || null,
                latestArtifactStatus: execution?.status || schedule?.lastRunStatus || null,
                dueTaskCount,
    ...
    async _probeContentOperationsCron() {
        ...
        return {
            ...
            safeDetails: { scheduleFound: Boolean(schedule), latestRunFound: Boolean(run), lastErrorPresent: Boolean(schedule?.lastError) }
        };
    }
    ...
    async _probeGoogleIntelligence() {
        ...
        } else if (!runtimeIssue && (
            ['partial', 'failed'].includes(snapshot.status) ||
            !snapshot.mandatorySourcesSucceeded ||
            schedule?.lastError
        )) {
            status = 'degraded';
    ...
    async _probeInventory() {
        ...
        return {
            ...
            warnings: snapshot
                ? (snapshot.warnings?.length || snapshot.errorCodes?.length
                    ? ['persisted_inventory_warning']
                    : snapshotFreshness.stale ? ['snapshot_stale'] : [])
    ...
    async _probeContentSignals() {
        ...
        const usable = Boolean(
            signal &&
            ['new', 'reviewed', 'used'].includes(String(signal.status || '')) &&
    ...
    async _probePerformanceMonitoring() {
        ...
        return {
            ...
            reasonCode: runtimeIssue?.reasonCode || (hasWarnings
            ...
            warnings: snapshot
                ? (snapshot.warnings?.length ? ['persisted_performance_warning'] : [])
                : ['no_performance_snapshot'],
            safeDetails: { latestSnapshotFound: Boolean(snapshot), warningCount: snapshot?.warnings?.length || 0 }
        };
    }
    ...
    async _probeLearning() {
        ...
        if (!config.enabled) {
            return {
                ...
                safeDetails: { latestRecordFound: Boolean(record), autoApply: config.learning.autoApply }
            };
        }
    ...
    async _probeImagePipeline() {
        const article = await this._latest(Blog, {
            select: 'imagePipelineStatus updatedAt',
            sort: { updatedAt: -1 }
        });
        const pipelineStatus = String(article?.imagePipelineStatus || '');
        return {
            status: pipelineStatus === 'complete' ? 'healthy' : pipelineStatus === 'failed' ? 'failed' : 'degraded',
            reasonCode: !article
            ...
            runtime: {
                serviceRegistered: true,
                lastSuccessfulRunAt: article?.updatedAt || null,
                latestArtifactStatus: article?.imagePipelineStatus || null
            },
            warnings: article ? [] : ['no_image_pipeline_history'],
            safeDetails: { latestResultFound: Boolean(article) }
        };
    }
    ...
    async _probeTelegram(definition) {
        ...
        return {
            ...
            runtime: {
                ...
                latestArtifactStatus: 'configuration_verified'
            },
    ...
    async _probeGateway() {
        const health = await this.gatewayProbe({ timeoutMs: Math.min(this.timeoutMs, 5000), env: this.envProvider() });
        return {
            status: health.ready ? 'healthy' : health.live ? 'degraded' : 'failed',
            reasonCode: health.ready ? 'runtime_verified' : health.error || 'gateway_not_ready',
            runtime: {
                serviceRegistered: Boolean(health.reachable),
                workerActive: Boolean(health.live),
                schedulerActive: false,
                lastHeartbeatAt: health.checkedAt,
                latestArtifactStatus: health.ready ? 'ready' : health.live ? 'live_not_ready' : 'unreachable'
            },
            safeDetails: {
    ...
    async _defaultProbe(definition) {
        switch (definition.dashboardKey) {
            case 'seoAgent': return this._probeExecutionHistory();
            case 'contentOperations': return this._probeContentOperations();
            case 'blogCron': return this._probeBlogCron();
            case 'contentOperationsCron': return this._probeContentOperationsCron();
            case 'googleIntelligence': return this._probeGoogleIntelligence();
            case 'searchConsole': return this._probePersistedSource('google_search_console');
            case 'aggregateAnalytics': return this._probePersistedSource('first_party_aggregate_analytics');
            case 'trends': return this._probePersistedSource('trends');
            case 'inventory': return this._probeInventory();
            case 'contentSignals': return this._probeContentSignals();
            case 'performanceMonitoring': return this._probePerformanceMonitoring();
            case 'learning': return this._probeLearning();
            case 'imagePipeline': return this._probeImagePipeline();
            case 'imageSearch':
            case 'aiImage':
                return {
                    status: 'degraded',
            ...
            case 'telegram': return this._probeTelegram(definition);
            case 'openclawGateway': return this._probeGateway();
            default:
