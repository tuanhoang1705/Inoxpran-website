...
const imageAttemptLimit = () =>
  boundedInteger(
    process.env.AI_IMAGE_PIPELINE_ATTEMPT_LIMIT,
    IMAGE_ATTEMPT_LIMIT,
    1,
    3,
  );
...
const imagePipelineConcurrency = () =>
  boundedInteger(
    process.env.AI_IMAGE_PIPELINE_CONCURRENCY,
...
const acquirePlanItemImage = async ({
  ...
  const resolved = await resolveSourceImage({ planItem, prompt, warnings });
  if (!resolved.source)
    throw new Error(resolved.reason || "no_image_provider_result");
