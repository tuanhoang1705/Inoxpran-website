...
const processPlanItem = async ({ planItem, slug, primaryKeyword, warnings }) => {
...
const runImagePipeline = async ({
    ...
    if (!enabled) {
        const pending = planItems.map((planItem) => pendingMetadata({
            planItem,
    ...
    for (const planItem of planItems) {
        try {
            results.push(await processPlanItem({ planItem, slug, primaryKeyword, warnings }));
        } catch (error) {
            warnings.push(error?.message || `${planItem.id}_image_pipeline_failed`);
            results.push(pendingMetadata({
                planItem,
                prompt: buildImagePrompt(planItem),
                seo: buildImageSeoMetadata({
                    articleTitle: planItem.articleTitle,
    ...
    const status = completeImages.length === results.length
        ? 'complete'
        : uploadedImages.length
            ...
            : results.some((item) => item.status === 'pending_generation')
                ? 'pending'
                : 'failed';
    const withInlineImages = insertInlineImages(contentHtml, contentImages.filter((item) => item.url));
    ...
    return {
        ...
        coverReadyForPublish: coverImage?.status === 'complete' &&
        ...
        publishReady: results.length > 0 && results.every((item) =>
